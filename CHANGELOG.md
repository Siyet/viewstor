...
### Changed
- **Result Grid: zebra striping + toolbar grouping** — added alternating row backgrounds via `--viewstor-row-zebra` token (matching Diff Panel), with proper specificity handling for new-row tint and hover; grouped toolbar items into logical clusters (status, search, export/visualize, row actions, pagination) separated by 1px vertical dividers ([#84](https://github.com/Siyet/viewstor/issues/84))
