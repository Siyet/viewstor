...
### Added
- **Apache Kafka driver** — browse topics, partitions and consumer groups in the tree; consume the last N messages of a topic into the Result Panel (right-click → Show Messages or `CONSUME <topic> [N] [from-beginning]`); produce single messages via `PRODUCE <topic> [key] <value>` (blocked on read-only connections); `LIST TOPICS` / `LIST GROUPS` / `DESCRIBE <topic>` command DSL. Host field accepts a comma-separated bootstrap broker list; username/password map to SASL/PLAIN with SSL toggle. Powered by `kafkajs` (pure JS, no native deps) ([#12](https://github.com/Siyet/viewstor/issues/12))
