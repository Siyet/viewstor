...
### Added
- **CodeMirror 6 SQL editor** — replaced the hand-rolled `<textarea>` + regex overlay tokenizer in the Result Panel query bar and Diff Panel SQL panes with a CodeMirror 6 editor. SQL syntax highlighting is now provided by `@codemirror/lang-sql` with dialect-aware parsing (PostgreSQL, SQLite, MySQL, MSSQL, ClickHouse). Inline table-name validation via `@codemirror/lint` highlights unknown tables when schema data is available. VS Code theme integration maps editor colors, selection, cursor, autocomplete widget, and diagnostics to `--vscode-*` CSS variables. Ctrl/Cmd+Enter runs the query; undo/redo history is built in. The diff panel's Synced toggle, Ctrl+Enter, and swap-sides all work through the new editor abstraction. Falls back to a plain `<textarea>` when the bundle is unavailable (e.g. in tests). Bundle adds ~375 KB (minified) to `dist/scripts/sql-editor.js` ([#90](https://github.com/Siyet/viewstor/issues/90))
