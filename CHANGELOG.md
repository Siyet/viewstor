...
### Added
- **Qdrant vector database driver** — browse collections, inspect vector configs and payload schemas, scroll/search/count points, upsert and delete via a command DSL (`LIST COLLECTIONS`, `DESCRIBE <collection>`, `SCROLL <collection>`, `SEARCH <collection> vector=[...] limit=N`, `COUNT <collection>`, `UPSERT <collection> id=<id> vector=[...]`, `DELETE <collection> id1 id2`). Connection form uses API key (password field); read-only mode blocks UPSERT/DELETE. MCP `add_connection` accepts `type: "qdrant"`. Pure JS client (`@qdrant/js-client-rest`), no native dependencies ([#16](https://github.com/Siyet/viewstor/issues/16))
