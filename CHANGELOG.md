...
### Added
- **MySQL / MariaDB driver** — full driver implementation using `mysql2`. Schema browser with tables, views, columns, indexes, triggers. DDL via `SHOW CREATE TABLE/VIEW/TRIGGER`. Autocomplete (tables, columns, databases, enum values). Safe mode (`EXPLAIN` + `ALL` type detection). Server-side pagination, inline editing, estimated row counts from `information_schema.TABLES`. Table statistics (data/index size, free space, avg row length, auto increment, engine). Chart time bucketing via `DATE_FORMAT()`. Grafana export. SSH tunnel and SOCKS5 proxy support. DBeaver/DataGrip import mapping. MCP and Chat participant integration ([#8](https://github.com/Siyet/viewstor/issues/8))
