...
- Saving a project-scope connection's SSH password or private key no longer wipes it back out of the current session moments later — VS Code's own file watcher reacts to the extension's own save (which, by design, writes the file *without* secrets) and was reloading from that file unconditionally.
