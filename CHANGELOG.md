...
### Added
- **Per-connection AI agent access mode** — `agentAccess` field on connections and folders (`full` / `schema-only` / `none`) gates both the in-process MCP commands and the standalone stdio MCP server. `schema-only` exposes `list_connections` / `get_schema` / `get_table_info` but blocks `execute_query`, `get_table_data`, `build_chart`, `export_grafana_dashboard`, `visualize`, `openQuery`, and `openTableData`. `none` hides the connection from `list_connections` and returns "not found" for every other tool so agents can't even discover it. Modes inherit down the folder tree (nearest ancestor wins, cycle-guarded). New Advanced dropdown in the connection/folder forms; restricted connections show an `[agents: schema-only]` / `[agents: none]` badge in the tree. New `viewstor.defaultAgentAccess` setting (default `full` so existing MCP users aren't affected) controls the fallback ([#71](https://github.com/Siyet/viewstor/issues/71))
