...
### Added
- **MySQL / MariaDB driver** — new `MysqlDriver` implementing the full `DatabaseDriver` interface via `mysql2` (Promise API, connection pooling, TLS). Schema browsing via `information_schema`, `SHOW CREATE TABLE/VIEW/TRIGGER` for DDL, `EXPLAIN`-based safe mode (detects `ALL` full table scans), `KILL QUERY` for cancel, estimated row counts from `information_schema.TABLES.TABLE_ROWS`, enum/set value suggestions in autocomplete, table statistics (data/index size, free space, engine, collation, auto increment), indexes/constraints/triggers in schema diff. Connection form shows MySQL/MariaDB option with default port 3306. Import from DBeaver/DataGrip recognizes MySQL and MariaDB connections. Chart time bucketing uses `DATE_FORMAT()`. MCP `add_connection` accepts `type: "mysql"` ([#8](https://github.com/Siyet/viewstor/issues/8))
