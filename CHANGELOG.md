...
### Added
- **On-demand database adapters** — database driver libraries (`pg`, `ioredis`, `@clickhouse/client`, `better-sqlite3`) are no longer bundled with the extension. On first connection of each type, Viewstor prompts to download the adapter (~2–8 MB via npm). Subsequent connections of the same type skip the download. `Viewstor: Manage Database Adapters` command (QuickPick) lists installed/missing adapters with install/uninstall actions. MCP `add_connection` auto-installs missing adapters and reports `{ installed: [...] }` in the response. All four drivers now use lazy `require()` through a central `AdapterManager` — no top-level imports of DB libraries, so a missing or broken adapter never blocks extension activation ([#76](https://github.com/Siyet/viewstor/issues/76))
