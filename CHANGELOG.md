...
### Added
- **Database statistics view** — new "Show Database Statistics" action on connection and database tree nodes (and command palette) opens a live panel showing overview tiles (database size, total tables / views / indexes), a sortable top-tables list with inline bars for row count / table size / index size / dead-tuple %, and connection-level metrics (active backends, commits, rollbacks, cache hit ratio, etc.). Respects hidden schemas, configurable `viewstor.databaseStats.topTablesLimit` (default 50, max 500) and `viewstor.databaseStats.autoRefreshSeconds` (default 0 = off, max 3600). Implemented for PostgreSQL (`pg_stat_*` / `pg_database_size`), ClickHouse (`system.tables` / `system.parts` / `system.processes`) and SQLite (`sqlite_master` + PRAGMAs, optional `dbstat` for per-table sizes) ([#73](https://github.com/Siyet/viewstor/issues/73))
