...
### Added
- **Common stats schema across all drivers** — every driver's `getTableStatistics` now emits the three canonical keys `row_count`, `total_size`, `last_modified` so cross-database-type comparisons always have a comparable baseline. Redis gains `getTableStatistics` (element count, `MEMORY USAGE`, type, TTL, encoding). SQLite adds `total_size` (table + index pages from `dbstat`). ClickHouse adds `last_modified` from `system.parts.modification_time`. PostgreSQL and SQLite emit `last_modified: null` since neither tracks per-table data-modification timestamps natively. Exported `COMMON_STAT_KEYS` constant for tooling ([#74](https://github.com/Siyet/viewstor/issues/74))
