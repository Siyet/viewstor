...
### Added
- **pgvector support (PostgreSQL)** — detects the `vector` extension and surfaces dimensions as `vector(N)` in the schema tree and `getTableInfo` instead of bare `vector`. SQL autocomplete gains pgvector distance operators (`<->`, `<#>`, `<=>`, `<+>`) and helper functions (`l2_distance`, `l1_distance`, `cosine_distance`, `inner_product`, `vector_dims`, `vector_norm`, `l2_normalize`) on pgvector-enabled connections ([#14](https://github.com/Siyet/viewstor/issues/14))
