...
### Added
- **Data diff: cross-DB-type Statistics tab** — previously hidden whenever the two sides were connections of different types (PG ↔ ClickHouse, SQLite ↔ PG, etc.). Now always populated when both drivers support `getTableStatistics`. In cross-type mode `computeStatsDiff` restricts the view to the intersection of metric keys (so a PG migration can be sanity-checked against a ClickHouse target without misleading empty cells for driver-specific metrics like `dead_tuples`), and an info banner above the stats tab reports how many per-side metrics were hidden (e.g. "Comparing postgresql ↔ clickhouse — showing 2 metrics common to both. 3 postgresql-specific and 2 clickhouse-specific metrics hidden.") ([#74](https://github.com/Siyet/viewstor/issues/74))
