...
### Changed
- **Shared SQL highlighter for Result Panel + Diff Panel** — extracted the inline SQL syntax highlighter into `src/webview/sqlHighlight.ts` (pure TS `highlightSql` + `SQL_HIGHLIGHT_SCRIPT` IIFE installing `window.ViewstorSql.highlightSql`). Both webviews now consume the same canonical source so a tweak to keywords or tokens can't drift between panels; parity between the TS and the IIFE is enforced by unit tests (step 1 of [#94](https://github.com/Siyet/viewstor/issues/94))
