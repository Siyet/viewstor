...
### Changed
- **Shared data-grid interaction layer for Result + Diff panels** — extracted cell selection (drag-select, Shift+click range, border rendering), copy formatting (TSV/CSV/Markdown/JSON/one-row), and context-menu wiring into `src/webview/scripts/data-grid.js` + `src/webview/styles/data-grid.css`. The Diff Panel's ~280 lines of duplicated selection/copy/menu code are replaced by a single `createSelectionManager()` call; the Result Panel's copy formatting now delegates to the shared `copyAsText()`. Selection border CSS (`cell-selected` + `sel-top/bottom/left/right`) lives in one file. Unit-tested via `node:vm` (19 tests in `src/test/dataGrid.test.ts`). Part of ([#94](https://github.com/Siyet/viewstor/issues/94))
