...
### Added
- **MCP agent write approval — human confirmation for destructive queries** — the in-process MCP `executeQuery` tool now classifies each SQL script (read / write / DDL / admin / unknown) and, for non-read queries, prompts the user with a VS Code modal before dispatch. Session approvals can be remembered per `(connection, risk kind)` for 5 minutes so an agent running a batch of similar writes isn't a prompt storm. The standalone stdio MCP server can't pop a modal, so it refuses writes with a structured `{ kind: 'approval_required' }` response unless the connection has `agentWriteApproval: 'never'` (explicit opt-out). New per-connection and per-folder `agentWriteApproval` setting (`always` / `ddl-and-admin` / `never`); default is `always`. Responses gained a `classification` field agents can reason about ([#75](https://github.com/Siyet/viewstor/issues/75))
