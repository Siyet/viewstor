...
### Changed
- **Data diff: panel-first UX for "Compare With..."** — right-clicking a table now opens the diff panel immediately with the source table loaded as the left pane; the right pane shows a "Pick a table to compare with" placeholder. The table picker opens on top automatically; if dismissed, the placeholder stays clickable so the user can re-invoke the picker without leaving the panel ([#74](https://github.com/Siyet/viewstor/issues/74))
