...
### Security
- **Standalone MCP `add_connection` gated by mode setting** — agents could bypass readonly/access restrictions by self-issuing connections via `add_connection`. New `viewstor.standaloneMcp.allowAddConnection` setting (or `VIEWSTOR_MCP_ADD_CONNECTION` env var) with three modes: `restricted` (default — forced `readonly: true`, project-scoped, `[agent]` prefix), `off` (tool hidden), `unrestricted` (today's behavior with a warning). Agent-created connections are marked with `agentCreated: true` and shown with an `(agent)` suffix in the tree view. Every call logged to `~/.viewstor/audit.log` ([#119](https://github.com/Siyet/viewstor/issues/119))
