...
### Changed
- **Result panel on @vscode-elements/elements + grid UX polish** — migrated the Result Panel toolbar, footer, export popup, and loading overlay to `vscode-button` / `vscode-textfield` / `vscode-single-select` / `vscode-icon` with codicons (`search`, `export`, `graph`, `map`, `add`, `remove`, `refresh`, `chevron-left`, `chevron-right`, `play`, `sync`, `close`); toolbar grouped into semantic sections (status | search | actions | spacer | editing | pagination) with subtle `1px` dividers via shared `--viewstor-border-subtle` token; grid data cells use monospace `--viewstor-font-mono` at `--viewstor-font-table-data-size`; zebra striping via `--viewstor-row-zebra` on even rows with row-number gutter tinted to match; NULL values styled with `--viewstor-text-dimmed` + italic; column type annotations use `--viewstor-font-type-annotation-size` with `0.8` opacity in a `col-type` span replacing the old `<br><small>` pattern; column width hints via `data-type-class` CSS attribute selectors (numeric ~80-120px, text ~150-250px, uuid ~290px, timestamp ~180px, boolean ~60px); row-number column fixed at `40px` with dimmed `--viewstor-font-meta-size` text; styles extracted to `src/webview/styles/result-panel.css` loaded alongside `tokens.css` + `codicon.css` via CSP-safe external links; all inline JS unchanged — no behavioral regression in messages, pagination, cell selection, search, context menu, JSON popup, export, editing, or sorting ([#89](https://github.com/Siyet/viewstor/issues/89))
