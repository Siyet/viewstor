...
### Added
- **N-way (multi-source) diff engine** — `computeNWayRowDiff`, `computeNWaySchemaDiff`, and `computeNWayStatsDiff` generalize the existing 2-source diff to N sources (up to 5 panes). Row diff uses a star topology — one reference source compared against all others — with per-source `sourceOnly` arrays and a `values[]` tuple per matched row. Schema diff compares type/nullable/PK/comment across N sources per column. Stats diff collects the intersection of metric keys across N stat sets, flagging presence and differences per source. `nwayToLegacyRowDiff` adapter converts N-way results back to the 2-source `RowDiffResult` shape for backwards compatibility. Fully unit-tested ([#74](https://github.com/Siyet/viewstor/issues/74))
