...
### Changed
- **Compare Data command opens panel-first** — `Viewstor: Compare Data` from the command palette now opens the diff panel immediately with two placeholder "Pick a table" panes instead of requiring both tables to be chosen upfront via sequential QuickPicks. Each pane opens the table picker on click; data loads and the full diff renders in place once both tables are selected. Part of ([#74](https://github.com/Siyet/viewstor/issues/74))
