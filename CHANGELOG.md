...
### Added
- **Neo4j driver** — connect to Neo4j graph databases via Bolt protocol (default port 7687, supports `neo4j://`, `neo4j+s://`). Run Cypher queries, browse node labels and relationship types, inspect node properties, view indexes and constraints. Schema tree shows Node Labels and Relationship Types as top-level groups. Node query results flatten properties into tabular rows. Autocomplete for labels, relationship types, and properties. Library: `neo4j-driver` ([#13](https://github.com/Siyet/viewstor/issues/13))
