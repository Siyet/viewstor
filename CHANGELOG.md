...
### Added
- **Interactive ER diagrams** — right-click a connected connection, database, or schema to visualize tables, views, and relationships on one continuous canvas. PostgreSQL schemas and ClickHouse databases occupy separate softly tinted, non-overlapping regions; SQLite keeps a plain canvas. Semantic zoom progresses from a compact relationship map to named square rectangles and then full cards with left-aligned columns and types, with columns appearing from `1.25×` of the overview zoom. Target-based wheel easing remains responsive to continuous trackpad input; each visible card is one local `Rect + Text` graphics group, so the camera scales its frame, glyphs, spacing, and padding through exactly the same transform. Semantic LOD updates no longer resend graph data or relationships on every animation frame. Cards use muted highlights for inline `PK`, `FK`, and `IDX` roles (including combined roles), while column hover shows comments and index names. At name-only zoom, holding over a table for three seconds previews its complete card. Hovering a table or relationship keeps its adjacent graph visible while unrelated objects fade through a fast 150 ms transition. Blank-canvas left-drag and middle-drag pan in both axes; wheel zoom works across the entire canvas, including beyond graph bounds. A relationship toggle and canvas legend clarify the graph; double-click isolates a table at the centre with its direct neighbours using fresh focused-layout coordinates, and `Esc` returns to the full graph. Views use a dashed purple border. Foreign-key arrows show column mappings and referential actions. PostgreSQL reads composite and cross-schema relationships from `information_schema`; SQLite uses `PRAGMA foreign_key_list`. Drivers without FK metadata still show the available relational structure with an explicit status note ([#6](https://github.com/Siyet/viewstor/issues/6))
