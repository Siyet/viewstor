...
### Added
- **Interactive ER diagrams** — right-click a connected connection, database, or schema to visualize tables, views, and relationships on one continuous canvas. Semantic zoom progresses from a compact relationship map to named square rectangles and then full cards with left-aligned columns and types. Primary keys are highlighted and marked inline; hovering a column shows its database comment when available. Hovering a table or relationship keeps its adjacent graph visible while unrelated objects fade through a fast 150 ms transition. Pan horizontally or vertically with a left-button drag or the middle mouse button; **Overview** resets the viewport. Views use a dashed purple border. Foreign-key arrows show column mappings and referential actions. PostgreSQL reads composite and cross-schema relationships from `information_schema`; SQLite uses `PRAGMA foreign_key_list`. Drivers without FK metadata still show the available relational structure with an explicit status note ([#6](https://github.com/Siyet/viewstor/issues/6))
