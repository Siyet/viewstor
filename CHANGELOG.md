...
### Added
- **Standalone MCP: `add_connection` access control** — the `add_connection` tool now operates in one of three modes controlled by `~/.viewstor/settings.json` or `VIEWSTOR_ALLOW_ADD_CONNECTION` env var: `off` (tool disabled), `restricted` (default — forces read-only, project-scoped, `[agent]`-prefixed connections), `unrestricted` (today's behavior with a warning). Prevents agents from bypassing per-connection access controls by self-issuing writeable connections to the same host. Agent-created connections are marked `mcpCreated: true` and show an `[agent]` badge in the VS Code tree view ([#119](https://github.com/Siyet/viewstor/issues/119))
