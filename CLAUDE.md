...
N-way (multi-source) diff: `computeNWayRowDiff(sources[], options)` uses a star topology — one reference source (default index 0) compared against all others. Returns `NWayRowDiffResult` with `values: (Record | null)[]` per matched row and `sourceOnly[][]` per source. `computeNWaySchemaDiff(columnSets[])` compares type/nullable/PK/comment across N sources per column name. `computeNWayStatsDiff(statSets[])` collects all metric keys and builds per-source value arrays with `present[]` and `hasDifferences` flags. `nwayToLegacyRowDiff()` converts N-way row results back to 2-source `RowDiffResult` for backward-compatible call sites.

`src/diff/diffTypes.ts` — `DiffSource`, `DiffOptions`, `RowDiffResult`, `MatchedRow`, `SchemaDiffResult`, `ColumnCompare`, `ObjectsDiffResult`, `ObjectDiffItem`, `StatsDiffResult`, `StatsDiffItem`. N-way types: `NWayDiffOptions` (extends `DiffOptions` with `referenceIndex`), `NWayMatchedRow`, `NWayRowDiffResult`, `NWayColumnCompare`, `NWaySchemaDiffResult`, `NWayStatsDiffItem`, `NWayStatsDiffResult`.
