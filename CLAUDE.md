...
### ER Diagram
`src/er/erDataTransform.ts` — pure transformation from nested `SchemaObject[]` + `ForeignKeyInfo[]` to flat graph tables and views. Extracts direct column children and their optional database comments, preserves schema-qualified ids and object kind, recognizes `(PK)` badges, and removes relationships whose endpoints are outside the selected schema scope. PostgreSQL and ClickHouse populate column comments in their batched schema queries.

`src/er/erDiagramPanel.ts` — `ErDiagramPanelManager`, an ECharts graph webview built on shared `tokens.css`, `@vscode-elements/elements`, and codicons. One panel/cache per connection + database + schema scope. The entire scope appears on one continuous canvas without a sidebar or selection layers. Semantic zoom moves from a far relationship map to named square table/view rectangles and then full cards with left-aligned columns. Primary keys are highlighted with `, PK` appended to their type; hovering a column shows its optional database comment in a custom DOM tooltip. At name-only zoom, a stationary three-second hover opens a full-card tooltip containing every column. Table/edge adjacency emphasis uses a 150 ms `cubicOut` state animation to fade unrelated nodes and links without delaying layout or zoom updates. The toolbar can hide/show relationships and a canvas legend documents graph notation. Double-click switches to a centred direct-neighbour graph; repeating it on the centre or pressing Escape restores the full scope. Custom graph-view translation makes blank-canvas LMB pan work even outside the outermost tables; middle-button pan uses the same path.
