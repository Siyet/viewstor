...
VS Code extension for database management. Supports PostgreSQL, Redis, ClickHouse, SQLite, Neo4j. Free, open-source (AGPL-3.0) alternative to DBeaver/DataGrip. Follows [ZeroVer](https://0ver.org) — version 0.x until API is stable.
...
Drivers: `postgres.ts` (pg), `redis.ts` (ioredis), `clickhouse.ts` (@clickhouse/client), `sqlite.ts` (better-sqlite3), `neo4j.ts` (neo4j-driver).
...
- Neo4j: graph database via Bolt protocol (`neo4j-driver`). Uses `neo4j://` or `neo4j+s://` (SSL) scheme. Schema: `CALL db.labels()` → node labels as "tables", `CALL db.relationshipTypes()` → relationship types; `CALL db.schema.nodeTypeProperties()` for column-like property discovery (falls back to sampling 100 nodes). `getTableData` runs `MATCH (n:Label) RETURN n SKIP/LIMIT`; single-node results flatten properties into tabular rows. `getTableObjects` uses `SHOW INDEXES` / `SHOW CONSTRAINTS`. No safe mode (Cypher, not SQL). Connection form hides multi-DB chips and hidden-schemas fields.
