...
Optional: `getTableRowCount`, `getEstimatedRowCount` (pg_class.reltuples / system.tables), `getDDL`, `cancelQuery` (PG: pg_cancel_backend, CH: AbortController), `getCompletions` (structured: table/view/column/schema with parent), `getIndexedColumns` (pg_index query), `getTableObjects` (indexes, constraints, triggers, sequences — used by data diff), `getTableStatistics` (row count, sizes, vacuum info, scan counters — used by stats diff tab; PG uses `pg_table_size`/`pg_indexes_size` + `pg_stat_user_tables`, CH uses `system.tables` + `system.parts`, SQLite uses `COUNT(*)` + optional `dbstat` vtable, Redis uses `MEMORY USAGE` + type-specific cardinality commands). All four drivers emit the normalized presence contract `COMMON_STAT_KEYS` (`row_count`, `total_size`, `last_modified`), with unavailable values as `null`; cross-type comparison still requires the separate semantic allowlist because storage metrics differ between engines.
...
Drivers: `postgres.ts` (pg), `redis.ts` (ioredis), `clickhouse.ts` (@clickhouse/client), `sqlite.ts` (better-sqlite3), `mongodb.ts` (mongodb).
...
- **`src/webview/scripts/sql-highlight.js`** — canonical lightweight SQL highlighter shared by the Result table query bar and both Diff query editors (#94). The plain IIFE installs `window.ViewstorSql.highlightSql` and exports the same frozen API through CommonJS for `node:vm` tests. Diff loads the copied runtime asset before `diff-panel.js`; Result inlines those exact sanitized bytes through `loadWebviewAsset()`. Do not introduce a parallel TypeScript or inline tokenizer.
...
### Data Diff
`src/diff/diffEngine.ts` — pure functions: `computeRowDiff()` matches rows by key columns (PK or user-specified), then compares explicit left/right column mappings, including differently named pairs; callers that omit mappings retain the legacy full-union behavior. Cross-driver comparison normalizes exact numerics, typed boolean↔0/1 flags, and equivalent UTC timestamp representations without coercing arbitrary text/numbers. `computeSchemaDiff()` compares column names, types, nullability, PK status. `computeObjectsDiff()` compares indexes, constraints, triggers, sequences. `computeStatsDiff(left, right, { crossType?, comparableMetrics? })` compares table-level statistics key-by-key, computes numeric delta + percent, and preserves `badWhen` only when both sides agree; cross-type mode deduplicates keys, validates units, and emits only metrics declared semantically comparable (currently row count by default), reporting omitted per-side counts in `summary.leftHiddenCount` / `summary.rightHiddenCount`. `formatStatValue()` formats bytes/count/percent/date. `buildDefaultDiffQuery()` generates the initial `SELECT * FROM <table> LIMIT <rowLimit>` for the editable-query UI. `exportDiffAsCsv()` / `exportDiffAsJson()` for diff export. No vscode dependency, fully unit-tested.

`src/diff/diffTypes.ts` — `DiffSource`, `DiffOptions`, `RowDiffResult`, `MatchedRow`, `SchemaDiffResult`, `ColumnCompare`, `ObjectsDiffResult`, `ObjectDiffItem`, `StatsDiffResult`, `StatsDiffSummary`, `StatsDiffItem`.

`src/diff/diffPanel.ts` — `DiffPanelManager`, webview panel for side-by-side diff visualization. Built on `@vscode-elements/elements` (`vscode-tabs` / `vscode-tab-header` / `vscode-tab-panel` / `vscode-collapsible` / `vscode-button` / `vscode-checkbox` / `vscode-icon`) + codicons + shared `tokens.css`. Tab headers carry custom `tab-badge` count chips that tint warn-color when the tab has real differences. Row Diff tab (added/removed/changed rows with cell-level highlighting; zebra striping via `--viewstor-row-zebra` on every tbody row, status classes win), Schema Diff tab (column comparison + objects, rendered as card blocks with the same zebra striping), Statistics tab (horizontal bar chart built with ECharts — each row normalized to its own max; zero-on-both-sides metrics collapse into an inline summary line rather than rendering empty chart cells; non-numeric "Other" metrics render as a card matching the chart/schema cards). Cross-type statistics show only explicitly comparable metrics, explain estimated row counts and omitted metrics in an accessible banner, and render an empty state when none are comparable. Sticky `diff-source-bar` under the tabs labels Left / Right once so per-column sub-headers don't repeat source labels. All filter chips default to active so the diff opens with the complete picture; click to solo / Shift+click to toggle. Filter chips use shared `--viewstor-badge-bg-*` tokens. CSS rule `vscode-tab-panel[hidden] { display: none }` is load-bearing — without it the `display: flex` on `vscode-tab-panel` overrides the native hidden attribute and inactive tabs render stacked. Export as CSV/JSON. Swap sides reuses the same panel, preserves edited queries, and invalidates stale in-flight query results. Loads `dist/scripts/echarts.min.js` only when stats are present. Row Diff tab has a `vscode-collapsible` titled "SQL" with per-side `<textarea>` + `vscode-checkbox` Synced toggle (with visible `lock` codicon while on) + "Run Diff" button — when both connections are the same type, sync defaults ON and mirrors edits between panes. `runDiffQuery` reconnects drivers when needed, executes both edited queries, and recomputes only the row diff; schema / objects / stats tabs stay bound to the original tables. Rejects with inline errors if the new result sets don't carry the key columns.

`src/commands/diffCommands.ts` — `viewstor.compareWith` (context menu on tables), `viewstor.compareData` (command palette). Auto-detects a complete primary key from either source when every key column exists on the other side. If neither side exposes a compatible key, the key picker contains only shared columns and preselects a conventional `id`/single `*_id` candidate. When schemas differ, a second picker shows exact pairs (selected), similar-name mappings, and left/right-only fields (unchecked); matching keys remain mandatory. ClickHouse `getTableInfo` resolves MergeTree primary-key membership through `system.columns.is_in_primary_key`, including the implicit primary key inherited from `ORDER BY`. Fetches data + objects + statistics from both sources whenever both drivers support `getTableStatistics`; cross-type comparisons are reduced by `DiffPanelManager` to the explicitly comparable metric contract and surface omitted counts via an info banner, then the diff panel opens.
...
### Tunnels
`src/connections/tunnel.ts` — SSH tunnel via `ssh2` library (port forwarding); supports chaining through an additional jump host (`proxy.sshHops`) by connecting each hop over the previous hop's `forwardOut()` stream (ssh2's `ConnectConfig.sock`). The local listener is only opened after every hop's `ready` event, and every hop is torn down if any hop errors, at connect time or later. SOCKS5 proxy via raw socket negotiation. Used by PostgreSQL driver when `config.proxy` is set. Project-scope connections (`.vscode/viewstor.json`) never persist any proxy secret (`sshPassword`/`sshPrivateKey`/`sshPassphrase`/`proxyPassword`, on every hop) — see `stripSecretsForProjectFile` in `connectionManager.ts`.
...
- MongoDB: document-oriented NoSQL. `execute()` parses `db.collection.operation(args)` shell syntax (find, findOne, aggregate, countDocuments, distinct, insertOne/Many, updateOne/Many, deleteOne/Many, createIndex, dropIndex) or bare collection name as shorthand for `find({})`. Documents flattened for tabular display: ObjectId→string, nested objects→JSON, arrays→JSON, BSON types coerced. Field types inferred by sampling first 100 documents. `options.authSource` stores the authentication database (default `admin`). `_admin.listDatabases` is a pseudo-collection for listing databases via admin API. Pure JS driver (`mongodb` npm), bundled by webpack (no externals entry needed).
