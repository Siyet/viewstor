...
Drivers: `postgres.ts` (pg), `mysql.ts` (mysql2), `redis.ts` (ioredis), `clickhouse.ts` (@clickhouse/client), `sqlite.ts` (better-sqlite3).
...
- MySQL/MariaDB: TCP connection via `mysql2` (Promise API). Uses `information_schema` for schema/columns/indexes/constraints/triggers. No schemas — tables are flat under the database. DDL via `SHOW CREATE TABLE/VIEW/TRIGGER`. `cancelQuery()` via `KILL QUERY <threadId>` on a separate connection. `getEstimatedRowCount()` from `information_schema.TABLES.TABLE_ROWS`. Safe mode uses `EXPLAIN` + detects `ALL` type for full table scans. Enum/set values parsed from `COLUMN_TYPE`. Chart time bucketing via `DATE_FORMAT()`, custom buckets via `FROM_UNIXTIME(FLOOR(UNIX_TIMESTAMP()/N)*N)`. Uses backtick quoting for identifiers.
