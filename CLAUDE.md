...
- **`src/webview/scripts/data-grid.js` + `src/webview/styles/data-grid.css`** — shared data-grid interaction layer (#94). IIFE installs `window.ViewstorDataGrid` with `copyAsText(headers, rows, format, opts?)` (pure copy formatter for TSV/CSV/Markdown/JSON/one-row), `buildCopyMenuItems(onCopy, extra?)` (standard 7-item copy menu for `ViewstorContextMenu`), and `createSelectionManager(opts)` (cell selection with drag-select, Shift+click range, Ctrl/Cmd+C, context menu, Escape-to-clear). CSS provides `cell-selected` + `sel-top/bottom/left/right` border classes via inset box-shadow. Diff Panel delegates all selection/copy to a single `createSelectionManager()` call; Result Panel uses `copyAsText` and `buildCopyMenuItems` for formatting. Also exports via `module.exports` for Node-side tests (see `src/test/dataGrid.test.ts`, loaded in `node:vm`). Diff Panel loads via `<script src>`; Result Panel inlines via `fs.readFileSync` (same triple-path resolution as context-menu).
