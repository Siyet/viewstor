...
- **`src/webview/sqlHighlight.ts`** — shared lightweight SQL tokenizer. Exports the pure TS `highlightSql(text)` for unit tests and `SQL_HIGHLIGHT_SCRIPT` (IIFE source string) consumed by the Result Panel (inlined in `buildResultHtml`) and the Diff Panel (inlined in `buildHtml`). Both panels read `window.ViewstorSql.highlightSql` at runtime so keyword/regex tweaks land once. Parity between TS and IIFE is enforced by `src/test/sqlHighlight.test.ts`.
