...
`src/er/erDiagramPanel.ts` — `ErDiagramPanelManager`, an ECharts graph webview built on shared `tokens.css`, `@vscode-elements/elements`, and codicons. One panel/cache per connection + database + schema scope. The entire scope appears on one continuous canvas without a sidebar or selection layers. Semantic zoom moves from a far relationship map to named square table/view rectangles and then full cards with left-aligned columns. Target-based exponential smoothing drives the native ECharts graph controller; hysteretic `map` / `names` / `details` LOD prevents threshold thrashing. A quantized 10% `visualScale` is the single source for symbol size, rich-text widths, fonts, line heights, and padding; the detail frame opens before column text appears. Steady zoom patches omit graph data and links, status DOM updates are throttled, overview edges are straight, and shadows appear only on emphasis. Cards append combined `PK`, `FK`, and `IDX` roles with muted colors; hovering a column shows its optional database comment, FK role, and index names in a custom DOM tooltip. At name-only zoom, a stationary three-second hover opens a full-card tooltip containing every column. Table/edge adjacency emphasis uses a 150 ms `cubicOut` state animation to fade unrelated nodes and links. The toolbar can hide/show relationships and a canvas legend documents graph notation. Double-click switches to a centred direct-neighbour graph; repeating it on the centre or pressing Escape restores the full scope. Capture-phase wheel and blank-canvas pan handlers call the native graph controller so node scaling, edges, labels, and the model remain synchronized across the full canvas.
