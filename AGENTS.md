...
`src/er/erDiagramPanel.ts` — `ErDiagramPanelManager`, an ECharts graph webview built on shared `tokens.css`, `@vscode-elements/elements`, and codicons. One panel/cache per connection + database + schema scope. The entire scope appears on one continuous canvas without a sidebar or selection layers. Semantic zoom moves from a far relationship map to named square table/view rectangles and then full cards with left-aligned columns. A 130 ms `requestAnimationFrame` tween smooths wheel zoom, a 140 ms ECharts update animation unfolds overview nodes into detail cards, and continued zoom scales detailed card geometry, fonts, line heights, and padding together. Cards append combined `PK`, `FK`, and `IDX` roles with muted colors; hovering a column shows its optional database comment, FK role, and index names in a custom DOM tooltip. At name-only zoom, a stationary three-second hover opens a full-card tooltip containing every column. Table/edge adjacency emphasis uses a 150 ms `cubicOut` state animation to fade unrelated nodes and links. The toolbar can hide/show relationships and a canvas legend documents graph notation. Double-click switches to a centred direct-neighbour graph; repeating it on the centre or pressing Escape restores the full scope. Custom graph-view translation makes blank-canvas LMB pan work even outside the outermost tables; middle-button pan uses the same path. A capture-phase wheel handler applies and synchronizes graph-view scaling across the full canvas instead of being limited by ECharts' graph-bounds pointer checker.
