...
`src/er/erDiagramPanel.ts` — `ErDiagramPanelManager`, an ECharts graph webview built on shared `tokens.css`, `@vscode-elements/elements`, and codicons. One panel/cache per connection + database + schema scope. The entire scope appears on one continuous canvas without a sidebar or selection layers. Semantic zoom moves from a far relationship map to named square table/view rectangles and then full cards with left-aligned columns. Primary keys are highlighted with `, PK` appended to their type; hovering a column shows its optional database comment in a custom DOM tooltip. Left-button drag uses ECharts roam, while a mouse fallback adds middle-button pan. **Overview** resets the viewport.
