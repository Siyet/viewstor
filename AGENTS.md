...
`src/er/erDiagramPanel.ts` — `ErDiagramPanelManager`, an ECharts graph webview built on shared `tokens.css`, `@vscode-elements/elements`, and codicons. One panel/cache per connection + database + schema scope. The entire scope appears on one continuous canvas without a sidebar or selection layers. Semantic zoom moves from a far relationship map to named square table/view rectangles and then full cards with left-aligned columns. Primary keys are highlighted with `, PK` appended to their type; hovering a column shows its optional database comment in a custom DOM tooltip. Table/edge adjacency emphasis uses a 150 ms `cubicOut` state animation to fade unrelated nodes and links without delaying layout or zoom updates. Left-button drag uses ECharts roam, while a mouse fallback adds middle-button pan. **Overview** resets the viewport.

`src/webview/scripts/er-diagram-layout.js` — deterministic, relationship-aware ordering plus a collision-free serpentine shelf layout for variable-size nodes. It derives readable overview/detail zoom thresholds from graph bounds and viewport size. The layout is shared with Node-side regression tests through a CommonJS export.
