...
### Data Diff
`src/diff/diffEngine.ts` — pure functions: `computeRowDiff()` matches rows by key columns (PK or user-specified), then compares explicit left/right column mappings, including differently named pairs; callers that omit mappings retain the legacy full-union behavior. Cross-driver comparison normalizes exact numerics, typed boolean↔0/1 flags, and equivalent UTC timestamp representations without coercing arbitrary text/numbers. `computeSchemaDiff()` compares column names, types, nullability, PK status. `computeObjectsDiff()` compares indexes, constraints, triggers, sequences. `computeStatsDiff()` compares table-level statistics key-by-key, computes numeric delta + percent, preserves `badWhen` hint for red/green coloring. `formatStatValue()` formats bytes/count/percent/date. `buildDefaultDiffQuery()` generates the initial `SELECT * FROM <table> LIMIT <rowLimit>` for the editable-query UI. `exportDiffAsCsv()` / `exportDiffAsJson()` for diff export. No vscode dependency, fully unit-tested.
