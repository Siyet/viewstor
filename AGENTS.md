...
Relationship lines use lower base opacity below `3×`; hover highlighting and tooltips use the same threshold as arrowheads. Toolbar search matches table, view, and column names; multiple matching tables are highlighted on the full graph, while a unique match opens that table's direct-neighbour graph.
