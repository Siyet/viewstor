...
Supports PostgreSQL (`Seq Scan`), MySQL (`ALL` type in EXPLAIN), SQLite (`SCAN TABLE` via `EXPLAIN QUERY PLAN`), and ClickHouse. Set globally in settings or per connection. Auto-adds `LIMIT` to SELECTs that don't have one.
...
| MySQL / MariaDB | TCP | [mysql2](https://www.npmjs.com/package/mysql2) |
