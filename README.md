...
- Right-click a table → **Compare With...** → the diff panel opens right away with the source table loaded on the left and a "Pick a table" placeholder on the right. The table picker opens on top; dismiss it and the placeholder stays clickable so you can re-pick from inside the panel.
